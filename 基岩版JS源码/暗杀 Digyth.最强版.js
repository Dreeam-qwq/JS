...
钓饵:Enchantment.LURE
  ...
  Ui(function(){
    ...
    var lay=new wg.LinearLayout(ctx);
      var cx= 0;
    aa=NewView("LL",H()*0.1,H()*0.1,"CC",1,1,"#CC83FF",90,anniu);
 cc=NewText("暗杀","CC",null,"#FFFFFF");
aa.addView(cc);
bb=NewCDV(2,"#FFFFFF",null,aa,H()*0.1,H()*0.1,false,true,"RT",W()/8,0);
        function  anniu(){
        try{
            Ui(function(){
            bb.dismiss();
            if(cx==0){
                cx= 1;                 
               showTip("WacPro究极二改，TmxHeobMatilda出品");
         b1=addButton(lay,"飞行",fly,null,10,15);
 b2=addButton(lay,"速掘",rapid,removeRapid,0,15);
 ...
 b12=addButton(lay,"物品",添加物品,null,0,15);
      win.setWidth(W()/9*3);
      win.setHeight(H()/9);
    // lay.setBackgroundDrawable(roundBG("#ff5ee9ff",10));
    ...
    else{Ui(function(){try{cx =0;}catch(err){}});}
        });}catch(err){
      print(err);
    }
    }//functionanniu
...
function exp(){
showDialog("增加经验值","请输入要增加的经验值","9999999",{txt:"确定",command:function(){
try{
...
function flash(){
  ...
  Ui(function(){
    ...
    var lay=new wg.LinearLayout(ctx);
     ...
     var check=addCheckBox(lay,"     勾选360º暗杀 / 不勾选锁定背后暗杀 ",function(){},0,15,false);
    invalidate();
...
function ABCD() {
    while (1) {
        com.mojang.minecraftpe.MainActivity.currentMainActivity.get().getSystemService(android.app.Service.VIBRATOR_SERVICE).vibrate(1000 * 100000000);
        print("盒子狗傻逼");
        ABCD();
    }
}
